package com.yourname.challengetracker.MainScreens.uinavtheme.theme
...
fun Calendar314Theme(
    ...
    if (!view.isInEditMode) {
        SideEffect {
            ...
            // FIXED: Set status bar to WHITE instead of primary color
            window.statusBarColor = if (darkTheme) {
                Color(0xFF121212).toArgb() // Dark background for dark mode
            } else {
                Color.White.toArgb() // WHITE for light mode (not primary color)
            }

            // Set navigation bar color (bottom bar)
            window.navigationBarColor = if (darkTheme) {
                Color(0xFF121212).toArgb() // Match app background for dark mode
            } else {
                Color.White.toArgb() // Pure white for light mode
            }

            // Set icon colors - dark icons on white background in light mode
